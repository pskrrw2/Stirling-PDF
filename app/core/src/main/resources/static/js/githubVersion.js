  ...
  if (updateSummary.latest_version && compareVersions(updateSummary.latest_version, currentVersion) > 0) {
    ...
    if (updateLinkLegacy !== null) {
      document.getElementById("app-update").innerHTML = updateAvailable
        .replace("{0}", "<b>" + currentVersion + "</b>")
        .replace("{1}", "<b>" + updateSummary.latest_version + "</b>");
      if (updateLinkLegacy.classList.contains("visually-hidden")) {
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable" role="document" style="max-height: 80vh;">
        <div class="modal-content" style="max-height: 80vh;">
              ...
              <div class="alert ${summaryData.max_priority === "urgent" ? "alert-danger" : "alert-warning"}" role="alert">
                ...
                ${
                  summaryData.recommended_action
                    ? `<br><strong>${updateRecommendedAction}:</strong> ${escapeHtml(summaryData.recommended_action)}`
                    : ""
                }
              </div>
            </div>
                ...
                <ul class="list-group">
                  ${summaryData.migration_guides
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                      ...
                      <a href="${escapeHtml(
                        guide.url
                      )}" target="_blank" class="btn btn-sm btn-outline-primary">${updateViewGuide}</a>
                    </li>
                  `
                    )
                    .join("")}
          ...
          <div class="modal-footer">
            ...
            <a href="#" target="_blank" class="btn btn-outline-primary">${updateViewAllReleases}</a>
            ${
              getDownloadUrl()
                ? `<a href="${escapeHtml(
                    getDownloadUrl()
                  )}" class="btn btn-success" target="_blank">${updateDownloadLatest}</a>`
                : ""
            }
          </div>
        </div>
            ...
            <div class="accordion-item" style="border-color: var(--md-sys-color-outline);">
                          <h2 class="accordion-header"  id="heading${index}">
                             <button class="accordion-button ${
                               index === 0 ? "" : "collapsed"
                             }" style="color: var(--md-sys-color-on-surface); background-color:
             var(--md-sys-color-surface);" type="button" data-bs-toggle="collapse"
                ...
                index === 0 ? "true" : "false"
                  ...
                  <div class="d-flex justify-content-between w-100 me-3">
                    ...
                    <span class="badge ${
                      version.priority === "urgent"
                        ? "bg-danger"
                        : version.priority === "normal"
                        ? "bg-warning"
                        : "bg-secondary"
                    }">${getTranslatedPriority(version.priority)}</span>
                  </div>
                </button>
              </h2>
             ...
             var(--md-sys-color-surface-bright);">
                  ...
                  ${
                    ...
                    <div class="alert alert-warning alert-sm" role="alert">
                      <small><strong>⚠️ ${updateBreakingChanges}</strong> ${
                          version.compatibility.breaking_description || updateBreakingChangesDefault
                        }</small>
                      ${
                        version.compatibility.migration_guide_url
                          ? `<br><a href="${version.compatibility.migration_guide_url}" target="_blank" class="btn btn-sm btn-outline-warning mt-1">${updateMigrationGuide}</a>`
                          : ""
                      }
                    </div>
                  `
